...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "update") {
      ...
      if (table !== "zone_danger" && table !== "news") {
        return jsonResponse({ error: "update not supported for this table" }, 400);
      }
      const allowed = table === "zone_danger" ? ZONE_PATCH_KEYS : new Set(["title", "summary", "source", "source_url", "image_source", "date", "channel_name", "channel_id"]);
      const patch = pickPatch(patchRaw, allowed);
