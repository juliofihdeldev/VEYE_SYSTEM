...
async function extractData(text: string, sourceMetadata: Record<string, unknown> = {}): Promise<Record<string, unknown>> {
	const { googleGeminiApiKey } = getConfig();
...
---`;
	...
	try {
		...
		const content = (typeof response.text === "function" ? response.text() : response.text) || (response as { candidates?: { content?: { parts?: { text?: string }[] } }[] }).candidates?.[0]?.content?.parts?.[0]?.text || "{}";
...
async function geocodeLocation(address: string): Promise<{ latitude: number; longitude: number; city: string | null } | null> {
	if (!address || address.trim().length < 2) return null;
	const { geocodingKey } = getConfig();
	if (!geocodingKey) return null;
	const addressWithHaiti = address.trim().toLowerCase().includes("haiti") ? address.trim() : `${address.trim()}, Haiti`;
	...
	try {
		...
		const cityComponent = components.find((c: { types?: string[] }) => c.types?.includes("locality") || c.types?.includes("administrative_area_level_2"));
...
async function getEmbedding(text: string): Promise<number[] | null> {
	...
	try {
		...
		const embedding = (response as { embeddings?: { values?: number[]; embedding?: number[] }[] }).embeddings?.[0]?.values ?? (response as { embeddings?: { values?: number[]; embedding?: number[] }[] }).embeddings?.[0]?.embedding ?? null;
...
async function isDuplicateEmbedding(supabase: SupabaseClient, embedding: number[] | null, _text: string): Promise<boolean> {
	if (!embedding?.length) return false;
	const cutoff = new Date(Date.now() - DEDUPE_LOOKBACK_HOURS * 3600 * 1000).toISOString();

	const { data: zones } = await supabase.from("zone_danger").select("id, rezon, name").gte("date", cutoff).order("date", { ascending: false }).limit(50);

	const { data: kidnaps } = await supabase.from("kidnaping_alert").select("id, rezon, name").gte("date", cutoff).order("date", { ascending: false }).limit(50);

	const candidates = [...(zones ?? []), ...(kidnaps ?? [])];

	for (const doc of candidates) {
		const id = doc.id as string;
		const { data: cached } = await supabase.from("ai_pipeline_embeddings").select("embedding").eq("source_doc_id", id).maybeSingle();
...
async function writeToDb(supabase: SupabaseClient, extracted: Record<string, unknown>, sourceMetadata: Record<string, unknown>): Promise<{ collection: string; id: string; zoneId?: string }> {
	...
	const { data: zoneIns, error: zErr } = await supabase.from("zone_danger").insert(zoneRow).select("id").single();
	...
	if (docType === "viktim" || incidentType === "kidnapping") {
		...
		const { data: kidIns, error: kErr } = await supabase.from("kidnaping_alert").insert(kidRow).select("id").single();
...
export async function isUserBlocked(supabase: SupabaseClient, userId: string): Promise<{ blocked: boolean; unblockedAt: number | null }> {
	const { data: row } = await supabase.from("user_moderations").select("*").eq("user_id", userId).maybeSingle();
...
async function recordSpamStrike(supabase: SupabaseClient, userId: string, reason: string): Promise<void> {
	const { data: doc } = await supabase.from("user_moderations").select("strikes").eq("user_id", userId).maybeSingle();

	const now = Date.now();
	const windowMs = SPAM_WINDOW_HOURS * 60 * 60 * 1000;
	let strikes: number[] = Array.isArray(doc?.strikes) ? (doc!.strikes as unknown[]).filter((x): x is number => typeof x === "number") : [];
...
export async function processPost(supabase: SupabaseClient, postPayload: Record<string, unknown>, isSentByUser = false): Promise<Record<string, unknown>> {
