...
async function getLastUpdateId(supabase: SupabaseClient): Promise<number> {
	const { data } = await supabase.from("telegram_monitor_state").select("last_update_id").eq("id", "cursor").maybeSingle();
...
function parseChannelPost(post: Record<string, unknown>): Record<string, unknown> {
	...
	const sourceUrl = chat.username ? `https://t.me/${chat.username}/${messageId}` : `https://t.me/c/${safeChannelId}/${messageId}`;
	const imageSource = Array.isArray(post.photo) && post.photo.length > 0 ? "photo_present" : null;
	return {
		source: "telegram",
		sourceUrl,
...
async function isTelegramRawDuplicate(supabase: SupabaseClient, channelId: number, messageId: number): Promise<boolean> {
...
async function insertNewsFallback(supabase: SupabaseClient, payload: Record<string, unknown>): Promise<void> {
...
export async function runTelegramMonitor(supabase: SupabaseClient, botToken: string, channelFilter: string[] | null): Promise<Record<string, unknown>> {
	...
	for (const update of updates) {
		...
		const pushDetail = (outcome: string, extra: Record<string, unknown> = {}) => {
			...
			const title = (chat?.title as string) || (chat?.username ? `@${chat.username}` : null) || (chatSource === "private" ? "Discussion privée" : null) || "—";
			updateDetails.push({
				updateId: uid,
				chatSource,
				chatTitle: title,
