...
- [x] **Profile tab** — RN parity: notifications (**POST_NOTIFICATIONS** on API 33+), dark theme, language (en/fr/ht), radii + Edge **`process-user-merge`** → Postgres **`users`** (with optional **`PROCESS_ALERT_SECRET`**), FAQ (eight items, en/fr/ht), privacy and about links, share, logout → new anonymous session + **FCM token re-bind** under the new uid
