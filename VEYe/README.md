...
- [x] **Report tab** — types, coords from DataStore map session, camera (**FileProvider**) + photo picker (≤4), victim name when **missing**, description / phone / anonymous; **block banner** from Edge **`get-user-moderation`** (polls Postgres `user_moderations` ~every **15 min**); **Supabase Edge** `process-global-alert` via **`supabase.functions.invoke`** (same Supabase URL/anon as the rest of the app); **PendingReport** strip on **Zones**; thank-you → **Zones** tab; prefill from alert detail
...
- [x] **Firebase** — Google Services plugin, BoM, **Auth** + **Analytics** + **Cloud Messaging (FCM)**; anonymous sign-in on startup (see `docs/FIREBASE_ANDROID.md`)
...
- [x] **Profile tab** — RN parity: notifications (**POST_NOTIFICATIONS** on API 33+), dark theme, language (en/fr/ht), radii + Edge **`process-user-merge`** → Postgres **`users`**, FAQ (eight items, en/fr/ht), privacy and about links, share, logout → new anonymous session + **FCM token re-bind** under the new uid
- [x] **Push (FCM)** — `firebase-messaging-ktx` + `VeyeFirebaseMessagingService`; current FCM token → Postgres **`users.device_token`** via **`process-user-merge`** (called from `VEYeApplication.onCreate` and on every `onNewToken` rotation); notification taps re-open `MainActivity` with `EXTRA_TARGET_TAB` from `data.target_tab` (RN parity); foreground messages built with our default channel `veye_alerts` and small icon `ic_stat_veye`
